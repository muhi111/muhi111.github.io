  ...
  return (
    <Box minH="100dvh" bg="gray.50" py={{ base: "2", md: "12" }}>
      <Box maxW={LAYOUT_CONSTANTS.MAX_CONTENT_WIDTH} mx="auto" px={{ base: "1", md: "4" }} w="full">
        <Heading as="h2" size="3xl" fontWeight="bold" color="gray.900" textAlign="center" mb="12">
          Works
        </Heading>
        <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} gap={{ base: "4", md: "6" }} w="full">
          {worksData.map((work) => (
            <WorkCard key={work.title} work={work} onClick={() => handleWorkClick(work)} />
          ))}
        </SimpleGrid>

        {isDetailsOpen && selectedWork && (
          <WorkDetails work={selectedWork} onClose={() => setIsDetailsOpen(false)} />
        )}
      </Box>
    </Box>
  );
