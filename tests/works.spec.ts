...
test.describe("Works画面", () => {
  ...
  test("ページタイトルが表示されること", async ({ page }) => {
    await expect(page.getByRole("heading", { name: "Works", level: 1, exact: true })).toBeVisible();
  });
  ...
  test("作品カードをクリックすると詳細モーダルが開き、正しい内容が表示されること", async ({
    ...
    for (const targetWork of worksData) {
      ...
      await expect(
        modal.getByRole("heading", { name: targetWork.title, level: 3, exact: true }),
      ).toBeVisible();
  ...
  test("詳細モーダルの閉じるボタンでモーダルが閉じること", async ({ page }) => {
    ...
    const cardTitle = page.getByRole("heading", { name: targetWork.title, level: 2, exact: true });
    await cardTitle.scrollIntoViewIfNeeded();
    await cardTitle.click();
  ...
  test("詳細モーダルの背景をクリックするとモーダルが閉じること", async ({ page }) => {
    const targetWork = worksData[0];
    const cardTitle = page.getByRole("heading", { name: targetWork.title, level: 2, exact: true });
    await cardTitle.scrollIntoViewIfNeeded();
    await cardTitle.click();
