...
test.describe("記事一覧画面", () => {
  ...
  test("すべての記事タイトルが表示されること", async ({ page }) => {
    for (const article of articlesData) {
      await expect(
        page.getByRole("heading", { name: article.title, level: 2, exact: true }),
      ).toBeVisible();
    }
  });

  test("すべての記事リンクが新しいタブで開く属性を持っていること", async ({ page }) => {
    for (const article of articlesData) {
      const link = page
        .getByRole("link")
        .filter({ has: page.getByRole("heading", { name: article.title, level: 2, exact: true }) });
